...
import com.qh.venus.achilles.framework.redis.annotation.RedisCache;
...
import com.qh.venus.achilles.sms.system.mapper.SysMenuMapper;
import com.qh.venus.achilles.sms.system.mapper.SysRoleMenuMapper;
...
public class SysMenuServiceImpl implements ISysMenuService {
	...
	public Map<Long, List<SysMenu>> selectMenusByUser(SysUser user) {
		List<SysTopMenu> sysTopMenu = sysTopMenuMapper.selectSysTopMenuList(new SysTopMenu());
		List<SysMenu> menus = new LinkedList<SysMenu>();
		Map<Long, List<SysMenu>> menusMap = new HashMap<Long, List<SysMenu>>();
		for (SysTopMenu topMenu : sysTopMenu) {
			// 管理员显示所有菜单信息
			if (user.isAdmin()) {
				menus = menuMapper.selectMenuNormalAll(topMenu.getId());
			} else {
				menus = menuMapper.selectMenusByUserId(user.getUserId(), topMenu.getId());
			}
			menusMap.put(topMenu.getId(),menus);
		}
		return menusMap;
		// 前端自行构造菜单树
	...
	public List<SysMenu> selectMenuIdsByTopMenuId(Long topMenuId) {
		return menuMapper.selectMenuIdsByTopMenuId(topMenuId);
	}
