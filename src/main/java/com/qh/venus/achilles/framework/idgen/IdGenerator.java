...
public final class IdGenerator {
	...
	static class IdGeneratorBuilder {
		...
		public IdGeneratorBuilder addHost(String host, int port, String pass, String luaSha) {
			...
			jedisPoolList.add(Pair.of(StringUtils.isEmpty(pass) ? new JedisPool(config, host, port, 1000)
					: new JedisPool(config, host, port, 1000, pass), luaSha));
			return this;
		}
	...
	static class LoadIdGeneratorConfig {
		...
		static {
			...
			try {
				...
				for (int i = 1; i <= 3; i++) {
					...
					scriptConf.add(new RedisScriptConfig(host,
							Integer.valueOf(pro.getProperty("redis_cluster" + i + "_port", "6379")), pass));
				}
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		public void loadScript() throws IOException {
			...
			for (RedisScriptConfig conf : scriptConf) {
				...
				InputStream is = LoadIdGeneratorConfig.class
						.getResourceAsStream("/script/redis-script-node" + index++ + ".lua");
