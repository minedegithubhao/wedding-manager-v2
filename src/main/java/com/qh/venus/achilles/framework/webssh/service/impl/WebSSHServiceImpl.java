package com.qh.venus.achilles.framework.webssh.service.impl;
...
import com.qh.venus.achilles.framework.webssh.pojo.SSHConnectInfo;
import com.qh.venus.achilles.framework.webssh.pojo.WebSSHData;
...
public class WebSSHServiceImpl implements WebSSHService {
    ...
    public void recvHandle(String buffer, WebSocketSession session) {
        ...
        if (ConstantPool.WEBSSH_OPERATE_CONNECT.equals(webSSHData.getOperate())) {
            ...
            SSHConnectInfo finalSshConnectInfo = sshConnectInfo;
            executorService.execute(new Runnable() {
                @Override
                public void run() {
                    try {
                        connectToSSH(finalSshConnectInfo, finalWebSSHData, session);
                    } catch (Exception e) {
                        logger.error("WebSSH登录失败");
                        logger.error("异常信息:{}", e.getMessage());
                        try {
                            sendMessage(session, "loginError".getBytes());
            ...
            if (sshConnectInfo != null) {
                try {
                    ...
                    logger.error("异常信息:{}", e.getMessage());
                    try {
                        sendMessage(session, "$ WebSSH->连接异常\r\n".getBytes());
                    } catch (IOException ex) {
                        ex.printStackTrace();
                    }
                    close(session);
